...
def docker_run(t,name,image):
    ...
    if state == "Up":
        ...
        cmd = ("docker run -w %s --cap-add=sys_ptrace -it --hostname=docker-`hostname`-`whoami` -h %s_%s -p 47000:7000 -p 47001:7001 -p 47002:7002 -p 47003:7003 -p 47004:7004 -p 47005:7005  -p 47006:7006 -p 47007:7007 -v /etc/localtime:/etc/localtime:ro -v %s:%s -v %s:%s --name %s_%s %s_%s:latest bash -c '/bin/bash;'")%(os.getcwd(),t,name,os.getenv("HOME"),os.getenv("HOME"),curRootDir,curRootDir,t,name,t,image)
    docmd(cmd)
