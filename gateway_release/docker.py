...
def docker_run(t,name,image):
    ...
    if state == "Up":
	    cmd = ("docker exec -ti %s_%s bash -c 'ulimit -c unlimited ;/bin/bash;'")%(t,name)
    elif state == "Exited" or state == "Created":
	    cmd = ("docker start %s_%s")%(t,name)
	    docmd(cmd)
	    cmd = ("docker exec -ti  %s_%s bash -c 'ulimit -c unlimited ;/bin/bash;'")%(t,name)
    elif platform.system().lower() == "darwin":
	    cmd = ("docker run -w %s --cap-add=sys_ptrace -it --hostname=docker-`hostname`-`whoami` -h %s_%s -v /etc/localtime:/etc/localtime:ro -v %s:%s --name %s_%s %s_%s:latest bash -c 'ulimit -c unlimited ;/bin/bash;'")%(os.getcwd(),t,name,os.getenv("HOME"),os.getenv("HOME"),t,name,t,image)
    else:
	    cmd = ("docker run -w %s --cap-add=sys_ptrace -it --hostname=docker-`hostname`-`whoami` -h %s_%s -p 46126:6026 -p 49100:9000 -p 49101:9001 -p 49102:9002 -p 49103:9003  -v /etc/localtime:/etc/localtime:ro -v %s:%s -v %s:%s --name %s_%s %s_%s:latest bash -c 'ulimit -c unlimited ;/bin/bash;'")%(os.getcwd(),t,name,os.getenv("HOME"),os.getenv("HOME"),curRootDir,curRootDir,t,name,t,image)
    docmd(cmd)
