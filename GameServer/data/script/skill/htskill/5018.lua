...
function OnUseSkill_5018(level,etype1,eid1,etype2,eid2,curdir)
    s_debug("onUseSkill 5018 dir")
	...
	--是否可PK
	--if s_checkPK(5018,etype1,eid1,etype2,eid2) == VALUE_OK then
	...
	for i = 1,table.getn(list), 2 do
		if (list[i] ~= etype1 or list[i+1] ~= eid1)  then
            if s_checkPK(5018,etype1,eid1,list[i],list[i+1]) == VALUE_OK then
    ...
    MagicAttack(5018,VALUE_TYPE_PDAM,etype1,eid1,unpack(victim))
	if etype1 == SCENE_ENTRY_PLAYER then
		s_addValue(etype1,eid1,VALUE_TYPE_SKILL_5018,1)
	end
end
