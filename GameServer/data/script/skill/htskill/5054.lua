--灵魂火符(对目标进行常规打击)
function OnUseSkill_5054(level,etype1,eid1,etype2,eid2)
	if s_getValue(etype1,eid1,VALUE_TYPE_PROFESSION) ~= Profession_Freeman then
		return USE_SKILL_ERROR
	end
    local damage  = GetAttackValue(etype1,eid1)
	local percent = {1.0500,1.0758,1.1017,1.1275,1.1533}
	--是否可PK
	if s_checkPK(5054,etype1,eid1,etype2,eid2) == VALUE_FAIL then
		return USE_SKILL_ERROR
	end
    ...
    local num    = 0;
	----是否可PK
    if s_checkPK(5054,etype1,eid1,etype2,eid2) == VALUE_OK then
	...
	--[[for i = 1,table.getn(list), 2 do
		if (list[i] ~= etype1 or list[i+1] ~= eid1)  then
            if (list[i] ~= etype2 or list[i+1] ~= eid2)  then
                ...
                if dst < 2 then
                    if s_checkPK(5054,etype1,eid1,list[i],list[i+1]) == VALUE_OK then
    ...
    MagicAttack(5054,VALUE_TYPE_MDAM,etype1,eid1,unpack(victim))
	if etype1 == SCENE_ENTRY_PLAYER then
		s_addValue(etype1,eid1,VALUE_TYPE_SKILL_5054,1)
	end
