...
function OnUseSkill_5037(level,etype1,eid1,etype2,eid2,curdir)
		...
		if (list[i] ~= etype1 or list[i+1] ~= eid1)  then
				...
				for j=1,#poslist,2 do
					if poslist[j] == entry_x and poslist[j+1] == entry_y then
						if s_checkPK(5037,etype1,eid1,list[i],list[i+1]) == VALUE_OK then
...
--s_debug("OnUseSkill_5037.....num="..num)
    MagicAttack(5037,VALUE_TYPE_MDAM,etype1,eid1,unpack(victim))
	if etype1 == SCENE_ENTRY_PLAYER then
		s_addValue(etype1,eid1,VALUE_TYPE_SKILL_5037,1)
	end
end
