...
function OnUseSkill_5039(level,etype1,eid1,etype2,eid2,posx,posy)
    ...
    local num    = 0;
	----是否可PK
    if s_checkPK(5039,etype1,eid1,etype2,eid2) == VALUE_OK then
	...
	for i = 1,table.getn(list), 2 do
		if (list[i] ~= etype1 or list[i+1] ~= eid1)  then
			if(list[i] ~= etype2 or list[i+1] ~= eid2) then
                    ...
                    local dst = math.sqrt((entry_x- att_x)^2 + (entry_y-att_y)^2)
                    if dst < 3 then
                        if s_checkPK(5039,etype1,eid1,list[i],list[i+1]) == VALUE_OK then
    ...
    MagicAttack(5039,VALUE_TYPE_MDAM,etype1,eid1,unpack(victim))

	if etype1 == SCENE_ENTRY_PLAYER then
		s_addValue(etype1,eid1,VALUE_TYPE_SKILL_5039,1)
	end
end
