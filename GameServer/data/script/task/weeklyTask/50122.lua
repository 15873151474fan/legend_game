...
--任务追踪
function RoleTaskInfo50122:GetTaskTraceText(uid)
...
--任务描述
function RoleTaskInfo50122:GetSubTaskDescText(uid,taskid,subtaskid)
	local text = [[title2(【%s】阿兰贝尔的幸运卡片)
	body(hotyellow_3(任务进程:)newline()space()space()神恩周常任务，请在本周内获得hot(]] .. godWeekTaskGetCardNum[self.taskID % 10] .. [[张抽奖卡片，周日即可交付领取任务奖励。)
    blankline()hotyellow_3(任务描述:)newline()space()space()已湮灭在历史中的亚特兰斯帝国最后的王子，阿兰贝尔被誉为给人们带来胜利的幸运之神，在一阵白色羽毛的围绕下，他要求你在hot(本周内拿到3张幸运抽奖卡片)，即可获得他的青睐！newline()space()space() hot(    (  每周都可参加随机神恩周常，周日才可以交付噢，根据任务星数的不同难度也不同，获得的奖励更不同啦blankline())))]]
...
--检查是否可以接受任务
function RoleTaskInfo50122:CheckAccept(uid,taskID)
...
function RoleTaskInfo50122:GetSubTaskAcceptText(uid,taskid,subtaskid)
	--确定收集内容
	local level = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_LEVEL)
...
function RoleTaskInfo50122:OnAcceptTask(uid)
	local level = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_LEVEL)
	if level < 101 then
...
--添加任务
function RoleTaskInfo50122:AddTask(uid)
...
--删除任务
function RoleTaskInfo50122:OnDeleteTask(uid)
	weeklyTaskFinishDeal(uid)
end

--检查是否显示完成菜单
function RoleTaskInfo50122:CheckFinish(uid)
	local process = s_getTaskValue(uid,self.taskID,TaskValue_Process)
	if process ~= TaskProcess_None then
		return VALUE_OK
	end
...
--显示交付任务菜单
function RoleTaskInfo50122:ShowFinish(uid)
	local process = s_getTaskValue(uid,self.taskID,TaskValue_Process)
	...
	elseif process == TaskProcess_CanDone then
		...
		local str = "RoleTaskInfo50122:FinishTask($1)"
		ShowNpcMenuBottomStyle(uid,msg,"完成(1)",str,"取消(1)")
	end
end

--避免CommonTaskReward
function RoleTaskInfo50122:TaskReward(uid, taskid, subtaskid)
end

--交付任务
function RoleTaskInfo50122:FinishTask(uid)
	...
	--神恩祈福道具
	s_addItem(uid,55276,0,2,0,"1-1","RoleTaskInfo50122")
...
--注册怪物回调
function RoleTaskInfo50122:RegisterMonsterCall(uid,level)
...
--RegisterNpcDeadLevelEvent("RoleTaskInfo50122:RegisterMonsterCall($1,$2)")

--未接取状态说明
function RoleTaskInfo50122:GetTaskIntroText(uid)
	local msg =
		[[
title(生存训练)
body(
...
--NPC菜单
--AppendMenuToNpc(60559,"生存训练(2)","RoleTaskInfo50122:ShowAccept($1)","RoleTaskInfo30045:CheckAccept($1)")
AppendMenuToNpc(372,"神恩周常任务(2)","RoleTaskInfo50122:ShowFinish($1)","RoleTaskInfo50122:CheckFinish($1)")
