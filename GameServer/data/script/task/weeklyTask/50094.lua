...
--任务追踪
function RoleTaskInfo50094:GetTaskTraceText(uid)
...
--任务描述
function RoleTaskInfo50094:GetSubTaskDescText(uid,taskid,subtaskid)
	local text = [[title3(【%s】赫淮斯托斯的火种)
    ...
    blankline()hotyellow_3(任务描述:)newline()space()space()你感觉到一股难以忍受的灼热感充斥在你的周围，一个燃烧着熊熊火焰的神出现在你的面前，他命令你在一周内击杀hot(6000)个怪物以熄灭他心中的怒火，事后赫淮斯托斯将给予你深藏在火种中的宝藏newline()（小道消息--听说奥丁大陆的怪物对阿佛洛狄忒不敬！）newline()hot(    每周都可参加随机神恩周常，周日才可以交付噢，根据任务星数的不同难度也不同，获得的奖励更不同啦！))]]
	local level = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_LEVEL)
	local mapName, mapid = getMapInfo(level)
...
--检查是否可以接受任务
function RoleTaskInfo50094:CheckAccept(uid,taskID)
...
function RoleTaskInfo50094:GetSubTaskAcceptText(uid,taskid,subtaskid)
	--确定收集内容
	local level = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_LEVEL)
...
function RoleTaskInfo50094:OnAcceptTask(uid)
	local level = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_LEVEL)
	if level < 101 then
...
--添加任务
function RoleTaskInfo50094:AddTask(uid)
...
--删除任务
function RoleTaskInfo50094:OnDeleteTask(uid)
	weeklyTaskFinishDeal(uid)
end

--检查是否显示完成菜单
function RoleTaskInfo50094:CheckFinish(uid)
	local process = s_getTaskValue(uid,self.taskID,TaskValue_Process)
	if process ~= TaskProcess_None then
		return VALUE_OK
	end
...
--显示交付任务菜单
function RoleTaskInfo50094:ShowFinish(uid)
	local process = s_getTaskValue(uid,self.taskID,TaskValue_Process)
	...
	elseif process == TaskProcess_CanDone then
		...
		local str = "RoleTaskInfo50094:FinishTask($1)"
		ShowNpcMenuBottomStyle(uid,msg,"完成(1)",str,"取消(1)")
	end
end

--避免CommonTaskReward
function RoleTaskInfo50094:TaskReward(uid, taskid, subtaskid)
end

--交付任务
function RoleTaskInfo50094:FinishTask(uid)
	...
	--神恩祈福道具
	s_addItem(uid,55276,0,4,0,"1-1","RoleTaskInfo50094")
	FinishRoleTask(uid,self.taskID)
	weeklyTaskFinishDeal(uid)
end

--注册怪物回调
function RoleTaskInfo50094:RegisterMonsterCall(uid,level)
...
RegisterNpcDeadLevelEvent("RoleTaskInfo50094:RegisterMonsterCall($1,$2)")

--未接取状态说明
function RoleTaskInfo50094:GetTaskIntroText(uid)
	local msg =
		[[
title(生存训练)
body(
...
--NPC菜单
--AppendMenuToNpc(60559,"生存训练(2)","RoleTaskInfo50094:ShowAccept($1)","RoleTaskInfo30045:CheckAccept($1)")
AppendMenuToNpc(372,"神恩周常任务(2)","RoleTaskInfo50094:ShowFinish($1)","RoleTaskInfo50094:CheckFinish($1)")
