--烈焰风暴
function OnUseSkill_10505(level,etype1,eid1,etype2,eid2,posx,posy)
	...
	local victim ={}
	--是否可PK
	if s_checkPK(10505,etype1,eid1,etype2,eid2) == VALUE_OK then
	...
	for i = 1,table.getn(list), 2 do
		if (list[i] ~= etype1 or list[i+1] ~= eid1)  then
			if(list[i] ~= etype2 or list[i+1] ~= eid2) then
				if s_checkPK(10505,etype1,eid1,list[i],list[i+1]) == VALUE_OK then
					...
					if s_getRuneDur(eid1,10505) > 0 then       --符文预留
	...
	if s_getRuneDur(eid1,10505) > 0 then
		s_reduceRuneDur(eid1,10505)
	end
	MagicAttack(10505,VALUE_TYPE_MDAM,etype1,eid1,unpack(victim))
end
