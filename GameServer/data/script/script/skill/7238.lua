--旋风斩
function OnUseSkill_7238(level,etype1,eid1,etype2,eid2)
	...
	for i = 1,table.getn(list), 2 do
		if (list[i] ~= etype1 or list[i+1] ~= eid1)  then
			if s_checkPK(7238,etype1,eid1,list[i],list[i+1]) == VALUE_OK then
	...
	--if s_getRuneDur(eid1,7238) > 0 then       --符文预留

	MagicAttack(7238,VALUE_TYPE_PDAM,etype1,eid1,unpack(victim))
end
