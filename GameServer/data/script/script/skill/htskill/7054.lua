...
function OnUseSkill_7054(level,etype1,eid1,etype2,eid2)
	...
	--是否可PK
	if s_checkPK(7054,etype1,eid1,etype2,eid2) == VALUE_FAIL then
		return USE_SKILL_ERROR
	end
    ...
    local num    = 0;
	----是否可PK
    if s_checkPK(7054,etype1,eid1,etype2,eid2) == VALUE_OK then
	...
	--[[for i = 1,table.getn(list), 2 do
		if (list[i] ~= etype1 or list[i+1] ~= eid1)  then
            if (list[i] ~= etype2 or list[i+1] ~= eid2)  then
                ...
                if dst < 2 then
                    if s_checkPK(7054,etype1,eid1,list[i],list[i+1]) == VALUE_OK then
    ...
    MagicAttack(7054,VALUE_TYPE_MDAM,etype1,eid1,unpack(victim))
	if etype1 == SCENE_ENTRY_PLAYER then
		s_addValue(etype1,eid1,VALUE_TYPE_SKILL_7054,1)
	end
