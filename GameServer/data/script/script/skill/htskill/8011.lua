--血战八方(攻击周围24个目标)
function OnUseSkill_8011(level,etype1,eid1,etype2,eid2,posx,posy)
    ...
    local num    = 0;
	--是否可PK
	if s_checkPK(8011,etype1,eid1,etype2,eid2) == VALUE_OK then
	...
	for i = 1,table.getn(list), 2 do
		if (list[i] ~= etype1 or list[i+1] ~= eid1)  then
			if(list[i] ~= etype2 or list[i+1] ~= eid2) then
					...
					local dst = math.sqrt((entry_x- att_x)^2 + (entry_y-att_y)^2)
                    if dst < 3 then
                        if s_checkPK(8011,etype1,eid1,list[i],list[i+1]) == VALUE_OK then
    ...
    MagicAttack(8011,VALUE_TYPE_PDAM,etype1,eid1,unpack(victim))
