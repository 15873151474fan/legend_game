...
function OnUseSkill_5014(level,etype1,eid1,etype2,eid2,curdir)
			...
			if(list[i] ~= etype2 or list[i+1] ~= eid2) then
                    ...
                    for j = 1,#poslist,2 do
                        if poslist[j] == entry_x and poslist[j+1] == entry_y then
								--s_trace(">>>>>>>>>>>>>>>>>kkkkkkkkk")
                            if s_checkPK(5014,etype1,eid1,list[i],list[i+1]) == VALUE_OK then
	...
	if etype1 == SCENE_ENTRY_PLAYER then
		s_addValue(etype1,eid1,VALUE_TYPE_SKILL_5014,1)
	end
    MagicAttack(5014,VALUE_TYPE_PDAM,etype1,eid1,unpack(victim))
