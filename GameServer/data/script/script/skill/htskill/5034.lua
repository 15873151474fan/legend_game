...
function OnUseSkill_5034(level,etype1,eid1,etype2,eid2,posx,posy)
		...
		if (list[i] ~= etype1 or list[i+1] ~= eid1)  then
				...
				for j=1,#poscycle1,2 do
					if poscycle1[j] == entry_x and poscycle1[j+1] == entry_y then
						if s_checkPK(5034,etype1,eid1,list[i],list[i+1]) == VALUE_OK then
    ...
    MagicAttack(5034,VALUE_TYPE_MDAM,etype1,eid1,unpack(victim))
    s_debug("5034>>>>>>>>>>>>>>>")
	if etype1 == SCENE_ENTRY_PLAYER then
		s_addValue(etype1,eid1,VALUE_TYPE_SKILL_5034,1)
	end
	--for i=1,#victim,3 do
