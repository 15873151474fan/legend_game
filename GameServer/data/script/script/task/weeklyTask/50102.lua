...
--任务追踪
function RoleTaskInfo50102:GetTaskTraceText(uid)
...
--任务描述
function RoleTaskInfo50102:GetSubTaskDescText(uid,taskid,subtaskid)
	local text = [[title2(【%s】伊阿珀托斯的灵魂收割)
	body(hotyellow_3(任务进程:)newline()space()space()神恩周常任务，请在本周内hot(击败敌对玩家]] .. godWeekTaskKillEnemyNum[self.taskID % 10] .. "名，周日即可交付领取任务奖励。" .. [[)
    blankline()hotyellow_3(任务描述:)newline()space()space()当伊阿珀托斯出现在你的面前时，你惊讶的无法自控，这是灵魂之神，一股幽冷的感觉充斥在你的心中，你知道了你必须hot(击败30个敌人)来献祭给灵魂之神，否则你将永无安宁之日。
...
--检查是否可以接受任务
function RoleTaskInfo50102:CheckAccept(uid,taskID)
...
function RoleTaskInfo50102:GetSubTaskAcceptText(uid,taskid,subtaskid)
	--确定收集内容
	local level = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_LEVEL)
	msg = parseToTalk(msg)
	msg = string.gsub(msg)
...
function RoleTaskInfo50102:OnAcceptTask(uid)
	local level = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_LEVEL)
	if level < 101 then
...
--添加任务
function RoleTaskInfo50102:AddTask(uid)
...
--删除任务
function RoleTaskInfo50102:OnDeleteTask(uid)
	weeklyTaskFinishDeal(uid)
end

--检查是否显示完成菜单
function RoleTaskInfo50102:CheckFinish(uid)
	local process = s_getTaskValue(uid,self.taskID,TaskValue_Process)
	if process ~= TaskProcess_None then
		return VALUE_OK
	end
...
--显示交付任务菜单
function RoleTaskInfo50102:ShowFinish(uid)
	local process = s_getTaskValue(uid,self.taskID,TaskValue_Process)
	...
	elseif process == TaskProcess_CanDone then
		...
		local str = "RoleTaskInfo50102:FinishTask($1)"
		ShowNpcMenuBottomStyle(uid,msg,"完成(1)",str,"取消(1)")
	end
end

--避免CommonTaskReward
function RoleTaskInfo50102:TaskReward(uid, taskid, subtaskid)
end

--交付任务
function RoleTaskInfo50102:FinishTask(uid)
	...
	--神恩祈福道具
	s_addItem(uid,55276,0,2,0,"1-1","RoleTaskInfo50102")
...
--注册怪物回调
function RoleTaskInfo50102:RegisterMonsterCall(uid,level)
...
--RegisterNpcDeadLevelEvent("RoleTaskInfo50102:RegisterMonsterCall($1,$2)")

--未接取状态说明
function RoleTaskInfo50102:GetTaskIntroText(uid)
	local msg =
		[[
title(生存训练)
body(
...
--NPC菜单
--AppendMenuToNpc(60559,"生存训练(2)","RoleTaskInfo50102:ShowAccept($1)","RoleTaskInfo30045:CheckAccept($1)")
AppendMenuToNpc(372,"神恩周常任务(2)","RoleTaskInfo50102:ShowFinish($1)","RoleTaskInfo50102:CheckFinish($1)")
