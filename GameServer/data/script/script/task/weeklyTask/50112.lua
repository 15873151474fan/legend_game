...
--任务追踪
function RoleTaskInfo50112:GetTaskTraceText(uid)
...
--任务描述
function RoleTaskInfo50112:GetSubTaskDescText(uid,taskid,subtaskid)
	local text = [[title2(【%s】德魔忒尔的点金术)
	body(hotyellow_3(任务进程:)newline()space()space()神恩周常任务，请在hot(本周内消费]] .. godWeekTaskUseMoney[self.taskID % 10] .. "赠点，周日即可交付领取任务奖励。" .. [[)
    blankline()hotyellow_3(任务描述:)newline()space()space()掌管着大地以及拥有神乎其技点金之术的德魔忒尔是奥丁大陆子民的最爱，为了每年的谷物的丰收以及立法保护，女神要求你献上hot(5000赠点)来换取她对你一如既往的祝福与保护。
...
--检查是否可以接受任务
function RoleTaskInfo50112:CheckAccept(uid,taskID)
...
function RoleTaskInfo50112:GetSubTaskAcceptText(uid,taskid,subtaskid)
	--确定收集内容
	local level = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_LEVEL)
...
function RoleTaskInfo50112:OnAcceptTask(uid)
	local level = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_LEVEL)
	if level < 101 then
...
--添加任务
function RoleTaskInfo50112:AddTask(uid)
...
--删除任务
function RoleTaskInfo50112:OnDeleteTask(uid)
	weeklyTaskFinishDeal(uid)
end

--检查是否显示完成菜单
function RoleTaskInfo50112:CheckFinish(uid)
	local process = s_getTaskValue(uid,self.taskID,TaskValue_Process)
	if process ~= TaskProcess_None then
		return VALUE_OK
	end
...
--显示交付任务菜单
function RoleTaskInfo50112:ShowFinish(uid)
	local process = s_getTaskValue(uid,self.taskID,TaskValue_Process)
	...
	elseif process == TaskProcess_CanDone then
		...
		local str = "RoleTaskInfo50112:FinishTask($1)"
		ShowNpcMenuBottomStyle(uid,msg,"完成(1)",str,"取消(1)")
	end
end

--避免CommonTaskReward
function RoleTaskInfo50112:TaskReward(uid, taskid, subtaskid)
end

--交付任务
function RoleTaskInfo50112:FinishTask(uid)
	...
	--神恩祈福道具
	s_addItem(uid,55276,0,2,0,"1-1","RoleTaskInfo50112")
...
--注册怪物回调
function RoleTaskInfo50112:RegisterMonsterCall(uid,level)
...
--RegisterNpcDeadLevelEvent("RoleTaskInfo50112:RegisterMonsterCall($1,$2)")

--未接取状态说明
function RoleTaskInfo50112:GetTaskIntroText(uid)
	local msg =
		[[
title(生存训练)
body(
...
--NPC菜单
--AppendMenuToNpc(60559,"生存训练(2)","RoleTaskInfo50112:ShowAccept($1)","RoleTaskInfo30045:CheckAccept($1)")
AppendMenuToNpc(372,"神恩周常任务(2)","RoleTaskInfo50112:ShowFinish($1)","RoleTaskInfo50112:CheckFinish($1)")
