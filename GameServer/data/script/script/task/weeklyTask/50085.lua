...
--任务追踪
function RoleTaskInfo50085:GetTaskTraceText(uid)
...
--任务描述
function RoleTaskInfo50085:GetSubTaskDescText(uid,taskid,subtaskid)
	local text = [[title4(【%s】牧神潘德活力迷宫)
	body(hotyellow_3(任务进程:)newline()space()space()神恩周常任务，请在本周内参与奥丁大陆的活动，达到hot(1050活力点)后，hot(周日)即可进行交付领取任务奖励。    blankline()hotyellow_3(任务描述:)newline()hot(space()space()森林的宠儿，优雅的牧神潘来到了你的身边，那是一个头上长着一对山羊角的半人半兽神，接着您耳边传来一阵低语，片刻后你突然明白了牧神潘的请求，他要求你在奥丁大陆上进行为期一周的历练，获得hout(1050点活力点)后换取牧神的赏赐！newline()space()space()hot(每周都可参加随机神恩周常，周日才可以交付噢，根据任务星数的不同难度也不同，获得的奖励更不同啦！)blankline()))]]
...
--检查是否可以接受任务
function RoleTaskInfo50085:CheckAccept(uid,taskID)
...
function RoleTaskInfo50085:GetSubTaskAcceptText(uid,taskid,subtaskid)
	--确定收集内容
	local level = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_LEVEL)
...
function RoleTaskInfo50085:OnAcceptTask(uid)
	local level = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_LEVEL)
	if level < 101 then
...
--添加任务
function RoleTaskInfo50085:AddTask(uid)
...
--删除任务
function RoleTaskInfo50085:OnDeleteTask(uid)
	weeklyTaskFinishDeal(uid)
end

--检查是否显示完成菜单
function RoleTaskInfo50085:CheckFinish(uid)
	local process = s_getTaskValue(uid,self.taskID,TaskValue_Process)
	if process ~= TaskProcess_None then
		return VALUE_OK
	end
...
--显示交付任务菜单
function RoleTaskInfo50085:ShowFinish(uid)
	local process = s_getTaskValue(uid,self.taskID,TaskValue_Process)
	...
	elseif process == TaskProcess_CanDone then
		...
		local str = "RoleTaskInfo50085:FinishTask($1)"
		ShowNpcMenuBottomStyle(uid,msg,"完成(1)",str,"取消(1)")
	end
end

--避免CommonTaskReward
function RoleTaskInfo50085:TaskReward(uid, taskid, subtaskid)
end

--交付任务
function RoleTaskInfo50085:FinishTask(uid)
	...
	--神恩祈福道具
	s_addItem(uid,55276,0,5,0,"1-1","RoleTaskInfo50085")
	FinishRoleTask(uid,self.taskID)
	weeklyTaskFinishDeal(uid)
end

--注册怪物回调
function RoleTaskInfo50085:RegisterMonsterCall(uid,level)
...
--RegisterNpcDeadLevelEvent("RoleTaskInfo50085:RegisterMonsterCall($1,$2)")

--未接取状态说明
function RoleTaskInfo50085:GetTaskIntroText(uid)
	local msg =
		[[
title(生存训练)
body(
...
--NPC菜单
--AppendMenuToNpc(60559,"生存训练(2)","RoleTaskInfo50085:ShowAccept($1)","RoleTaskInfo30045:CheckAccept($1)")
AppendMenuToNpc(372,"神恩周常任务(2)","RoleTaskInfo50085:ShowFinish($1)","RoleTaskInfo50085:CheckFinish($1)")
