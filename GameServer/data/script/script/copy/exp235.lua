...
function CopySceneInfo_236:InitCopyScene(cid)
...
function CopySceneInfo_236:ClearNpc(cid)
    --清空npc
    local list = s_getCopyEntry(cid,SCENE_ENTRY_NPC)
    for i = 1,table.getn(list), 2 do --清理NPC
        s_clearNpc(list[i+1])
...
--		s_debug("奖励波数--------"..s_getVar(uid,470,1))
    ...
    --开始刷怪
    if s_getSceneVar(cid,1,defencetower_isbegin) == 1 and s_getSceneVar(cid,1,defencetower_is_over) == 0 and s_getSceneVar(cid,1,defencetower_monster_current) <= 3 and curtime < s_getSceneVar(cid,1,defencetower_over_time) and s_getVar(uid,476,1) == 0 then
        if s_getSceneVar(cid,1,defencetower_monster_current) == 0 then
            s_setSceneVar(cid,1,defencetower_monster_current,1)
        end
        CopySceneInfo_236:beginRefreshMonster(s_getSceneVar(cid,1,defencetower_monster_current),cid)
    ...
    if monstercount == 0 and s_getSceneVar(cid,1,defencetower_monster_current) ~= 0 and s_getSceneVar(cid,1,defencetower_monster_current) <= 3 and curtime < s_getSceneVar(cid,1,defencetower_over_time) then
            ...
            if current > 3 then
		s_debug("停止刷新")
		if(s_getVar(uid,470,1) == 2) then
			s_setVar(uid,470,1,3)
		end
		s_setSceneVar(cid,1,defencetower_is_over,1)
...
--玩家复活
function CopySceneInfo_236:OnPlayerRelive(uid, rtype)
...
function sendExpReward(uid,itemid,exp)
        local flag = s_getVar(uid,470,3)
        s_debug("领取经验副本奖励22222222222,flag="..flag);
        if flag==0 then
            return
        end
	...
	s_setVar(uid,470,3,0)
        s_copyInfo(uid,"领取经验副本奖励成功")
end

--召唤守卫
function CopySceneInfo_236:RefreshNpc(typeid,cid,uid)
	s_debug("召唤兽为")
	if typeid == 4 then
		s_getVar(uid,474,1)
		...
		s_setValue(SCENE_ENTRY_NPC,npcid,VALUE_TYPE_MASTER,uid)
--			s_removeMoney(uid,MoneyType_Money,50,"召唤守卫扣除")
...
--			s_removeMoney(uid,MoneyType_Money,50,"召唤守卫扣除")
		...
		s_setValue(SCENE_ENTRY_NPC,npcid,VALUE_TYPE_MASTER,uid)
			s_debug("iiiiiii"..self.npc_pos[nulltab[1]][1].."0000000"..self.npc_pos[nulltab[1]][2])
--			s_removeMoney(uid,MoneyType_Gold,100,"召唤守卫扣除")
...
--玩家进入副本提示
function CopySceneInfo_236:OnPlayerIntoCopyScene(cid,uid)
    ...
    s_sendCopyCustomTime(cid,3,4)
    s_sendCopyCustomTime(cid,1,8)
...
function CopySceneInfo_236:beginRefreshMonster(curnumber,cid)
...
function CopySceneInfo_236:dead(cid,npcid)
        local list = s_getCopyEntry(cid,SCENE_ENTRY_NPC)
        local num = 0
...
for i=0,24 do
   local id = 22100 + i
   RegisterCopyNpcDeadEvent(22100+i,236,0,"CopySceneInfo_236:dead($1,$2)")
end
