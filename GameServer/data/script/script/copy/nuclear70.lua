...
function CopySceneInfo_32:OnPlayerRelive(uid, rtype)
	if rtype == 3 or rtype == 4 then
		s_goto(SCENE_ENTRY_PLAYER,uid,125,198)
	end
end

function OnLoopCopyScene_1Sec_32(ctype,cid)
	local last = s_getCurTime(TIME_ALLSECONDS) - s_getSceneVar(cid,0,1)
	...
	if begin > 0 then
		...
		if begin == 1 then
			...
			CopySceneInfo_32:createMonster(cid)
			local last = s_getCurTime(TIME_ALLSECONDS) - s_getSceneVar(cid,0,1)
			s_sendCopyInfo(cid,180,30*60 - last,"杀死电锯改进型")
			return
		end
...
function CopySceneInfo_32:createMonster(cid)
	local monster = {
		{22467,124,150,2,15,1},		{22464,148,96,10,10,1},
		{22465,148,96,10,10,5},		{22464,144,79,10,10,1},		{22465,144,79,10,10,5},
		{22464,103,79,10,10,1},		{22465,103,79,10,10,5},		{22464,97,97,10,10,1},
		{22465,97,97,10,10,5},		{22464,123,97,10,10,1},		{22465,123,97,10,10,5},
		{22464,122,55,5,10,2},		{22465,122,55,5,10,2}, }
...
function CopySceneInfo_32:dead22467(uid)--看门boss
	...
	if ctype >= 30 and ctype <= 39 then
		...
		if s_getSceneVar(cid,0,3) ==0 then
			local npcid = s_sysSummon(22466,cid,122,53)
...
function CopySceneInfo_32:dead22466(uid) --最终boss
	local cid = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_SCENE_ID)
...
-- RegisterMyNpcDeadEvent(22466,"CopySceneInfo_32:dead22466($1)")
-- RegisterMyNpcDeadEvent(22467,"CopySceneInfo_32:dead22467($1)")
