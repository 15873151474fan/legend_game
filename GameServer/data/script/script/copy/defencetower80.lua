...
defencetower_over_time_181                = 2;        --活动结束时间
...
defencetower_shudo_over 	  = 12;      --副本是否应该结束
--2 --xx玩家所获得的积分
...
--副本创建初始化
function CopySceneInfo_181:InitCopyScene(cid)
	s_setSceneVar(cid,1,defencetower_over_time_181,s_getCurTime(TIME_ALLSECONDS) + 1080)
...
--玩家死亡
function CopySceneInfo_181:OnPlayerDie(cid,uid,murder,dtype)

    --ReturnUserHome(uid,"CopySceneInfo_181:OnPlayerDie")
end

--销毁副本
function CopySceneInfo_181:FinalCopyScene(cid)

end

--清空npc
function CopySceneInfo_181:ClearNpc(cid)
    --清空npc
...
--1秒钟循环
function OnLoopCopyScene_1Sec_181(ctype,cid)
	...
	local ss = s_getSceneVar(cid,1,defencetower_shudo_over)
    --已经失败了，给个倒计时然后清理副本
    ...
    if s_getSceneVar(cid,1,defencetower_isbegin) == 1  and s_getSceneVar(cid,1,defencetower_shudo_over) == 0 then
        ...
        CopySceneInfo_181:beginRefreshMonster(s_getSceneVar(cid,1,defencetower_monster_current),cid)
        s_debug("开始刷怪,当前波数:"..s_getSceneVar(cid,1,defencetower_monster_current))
    end

    --检测是否进行下一波刷怪
    if s_getSceneVar(cid,1,defencetower_next_time) ~= 0 then
        s_debug("检测下一波:80还有秒数:"..s_getSceneVar(cid,1,defencetower_next_time) - curtime)
		s_debug("场景id "..cid)
...
--3秒钟循环
function OnLoopCopyScene_3Sec_181(ctype,cid)
    ...
    --检测活动时间结束
    if curtime >= s_getSceneVar(cid,1,defencetower_over_time_181) and s_getSceneVar(cid,1,defencetower_shudo_over) == 0 then
        --时间到了，npc还活着说明已经结束了
        if s_isNpcDead(cid,8401) == VALUE_FAIL then
            s_setSceneVar(cid,1,defencetower_shudo_over,1)
            s_setSceneVar(cid,1,defencetower_recycle_over,20)
            s_copyInfo(cid,"保护图腾成功",ChatPos_Tips)
            s_sendCopyCustomTime(cid,2,2)
...
function CopySceneInfo_181:beginRefreshMonster(curnumber,cid)
    ...
    if  self.route1_monster[curnumber] ~= nil then
        ...
        --s_debug("设置下一波开始时间.....1 npcbaseid = "..npcbaseid)
	s_debug("设置下一波开始时间.....1 cid = "..cid)
...
--玩家进入副本提示
function CopySceneInfo_181:OnPlayerIntoCopyScene(cid,uid)
	s_setValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_PK_MODE,1)

    local overtime = s_getSceneVar(cid,1,defencetower_over_time_181) - s_getCurTime(TIME_ALLSECONDS)
	...
	if taskProcess == TaskProcess_Doing then
        local curnum = s_getField(uid,UserVar_CTeamTimes1)
        if curnum > 0 then
            s_subField(uid, UserVar_CTeamTimes1, 1)
        end
    end
...
--玩家离开副本
function CopySceneInfo_181:OnPlayerLeaveCopyScene(cid,uid)
	s_setValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_PK_MODE,1)
    --完成主线任务
	local taskProcess = s_getTaskValue(uid, 1135, TaskValue_Process)
	if taskProcess == TaskProcess_Doing then
...
--怪物死后掉出宝箱
function CopySceneInfo_181:dead_7500(userid,npcid)
...
function CopySceneInfo_181:dead8300_8515(cid,npcthisid)
...
function CopySceneInfo_181:dead_8401(cid,npcthisid)
    s_copyInfo(cid,"保护图腾失败",ChatPos_Tips)
    s_setSceneVar(cid,1,defencetower_shudo_over,1)
    s_setSceneVar(cid,1,defencetower_recycle_over,20)
    s_sendCopyCustomTime(cid,1,2)
...
--玩家复活
function CopySceneInfo_181:OnPlayerRelive(uid, rtype)
...
--小怪死亡回调
function CopySceneInfo_181:CopySceneInfo_181_monster_death_callback(uid,npcid)
	local cid = s_getValue(SCENE_ENTRY_NPC,npcid,VALUE_TYPE_SCENE_ID)
...
--boss死亡回调
function CopySceneInfo_181:CopySceneInfo_181_boss_death_callback(uid,npcid)
	local cid = s_getValue(SCENE_ENTRY_NPC,npcid,VALUE_TYPE_SCENE_ID)
    local baseid = s_getValue(SCENE_ENTRY_NPC,npcid,VALUE_TYPE_BASE_ID)
    local point = 0
    if self.monster_point[baseid] ~= nil then
...
--守护图腾死亡回调
function CopySceneInfo_181_guard_death_callback(uid,npcid)
	local cid = s_getValue(SCENE_ENTRY_NPC,npcid,VALUE_TYPE_SCENE_ID)
    s_debug("图腾被杀死了")
    s_copyInfo(cid,"保护图腾失败",ChatPos_Tips)
    s_setSceneVar(cid,1,defencetower_shudo_over,1)
...
--守护图腾死亡事件
RegisterCopyNpcDeadEvent(8401,181,0,"CopySceneInfo_181:dead_8401($1,$2)")
--普通怪物死亡事件

for i=0,9 do
    RegisterMyNpcDeadEvent(8300+i,"CopySceneInfo_181:CopySceneInfo_181_monster_death_callback($1,$2)")
    RegisterMyNpcDeadEvent(8330+i,"CopySceneInfo_181:CopySceneInfo_181_monster_death_callback($1,$2)")
    RegisterMyNpcDeadEvent(8360+i,"CopySceneInfo_181:CopySceneInfo_181_monster_death_callback($1,$2)")
    --boss回调
    if i < 5 then
        RegisterMyNpcDeadEvent(8501+i,"CopySceneInfo_181:CopySceneInfo_181_boss_death_callback($1,$2)")
    end
end
...
--RegisterCopyNpcDeadEvent(8330,181,181,"CopySceneInfo_181:dead8300_8515($1,$2)")
--RegisterCopyNpcDeadEvent(8360,181,181,"CopySceneInfo_181:dead8300_8515($1,$2)")
--RegisterCopyNpcDeadEvent(8331,181,181,"CopySceneInfo_181:dead8300_8515($1,$2)")
