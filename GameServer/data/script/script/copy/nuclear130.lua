...
CopySceneInfo_38 = {}
--副本创建初始化
function CopySceneInfo_38:InitCopyScene(cid)
	s_setSceneVar(cid,0,0,5)
	s_setSceneVar(cid,0,1,s_getCurTime(TIME_ALLSECONDS))
end

function CopySceneInfo_38:OnPlayerIntoCopyScene(cid,uid)
...
function CopySceneInfo_38:OnPlayerRelive(uid, rtype)
	if rtype == 3 or rtype == 4 then
		s_goto(SCENE_ENTRY_PLAYER,uid,125,198)
	end
end

function OnLoopCopyScene_1Sec_38(ctype,cid)
	local last = s_getCurTime(TIME_ALLSECONDS) - s_getSceneVar(cid,0,1)
	...
	if begin > 0 then
		...
		if begin == 1 then
			...
			CopySceneInfo_38:createMonster(cid)
			local last = s_getCurTime(TIME_ALLSECONDS) - s_getSceneVar(cid,0,1)
			s_sendCopyInfo(cid,180,30*60 - last,"杀死电锯改进型")
			return
		end
...
function CopySceneInfo_38:createMonster(cid)
	local monster = {
		{22491,124,150,2,15,1},		{22488,148,96,10,10,1},
		{22489,148,96,10,10,5},		{22488,144,79,10,10,1},		{22489,144,79,10,10,5},
		{22488,103,79,10,10,1},		{22489,103,79,10,10,5},		{22488,97,97,10,10,1},
		{22489,97,97,10,10,5},		{22488,123,97,10,10,1},		{22489,123,97,10,10,5},
		{22488,122,55,5,10,2},		{22489,122,55,5,10,2}, }
...
function CopySceneInfo_38:dead22491(uid)--看门boss
	...
	if ctype >= 30 and ctype <= 39 then
		...
		if s_getSceneVar(cid,0,3) ==0 then
			local npcid = s_sysSummon(22490,cid,122,53)
...
function CopySceneInfo_38:dead22490(uid) --最终boss
	local cid = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_SCENE_ID)
...
-- RegisterMyNpcDeadEvent(22490,"CopySceneInfo_38:dead22490($1)")
-- RegisterMyNpcDeadEvent(22491,"CopySceneInfo_38:dead22491($1)")
