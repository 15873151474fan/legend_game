...
function CopySceneInfo_37:OnPlayerRelive(uid, rtype)
	if rtype == 3 or rtype == 4 then
		s_goto(SCENE_ENTRY_PLAYER,uid,125,198)
	end
end

function OnLoopCopyScene_1Sec_37(ctype,cid)
	local last = s_getCurTime(TIME_ALLSECONDS) - s_getSceneVar(cid,0,1)
	...
	if begin > 0 then
		...
		if begin == 1 then
			...
			CopySceneInfo_37:createMonster(cid)
			local last = s_getCurTime(TIME_ALLSECONDS) - s_getSceneVar(cid,0,1)
			s_sendCopyInfo(cid,180,30*60 - last,"杀死电锯改进型")
			return
		end
...
function CopySceneInfo_37:createMonster(cid)
	local monster = {
		{22487,124,150,2,15,1},		{22484,148,96,10,10,1},
		{22485,148,96,10,10,5},		{22484,144,79,10,10,1},		{22485,144,79,10,10,5},
		{22484,103,79,10,10,1},		{22485,103,79,10,10,5},		{22484,97,97,10,10,1},
		{22485,97,97,10,10,5},		{22484,123,97,10,10,1},		{22485,123,97,10,10,5},
		{22484,122,55,5,10,2},		{22485,122,55,5,10,2}, }
...
function CopySceneInfo_37:dead22487(uid)--看门boss
	...
	if ctype >= 30 and ctype <= 39 then
		...
		for i = 1,table.getn(list), 2 do
			...
			s_refreshExp(list[i+1],num,ExpType_Nuclear,60)
		end
		s_setSceneVar(cid,0,10,2)
		if s_getSceneVar(cid,0,3) ==0 then
			local npcid = s_sysSummon(22486,cid,122,53)
...
function CopySceneInfo_37:dead22486(uid) --最终boss
	local cid = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_SCENE_ID)
...
-- RegisterMyNpcDeadEvent(22486,"CopySceneInfo_37:dead22486($1)")
-- RegisterMyNpcDeadEvent(22487,"CopySceneInfo_37:dead22487($1)")
