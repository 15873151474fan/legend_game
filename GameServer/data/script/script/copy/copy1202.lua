...
function CopySceneInfo_1202:OnPlayerIntoCopyScene(sceneid, userid)
...
function CopySceneInfo_1202:CreateRobot(sceneid, userid)
...
function CopySceneInfo_1202:ClearRobot(sceneid, userid)
	local viplevel = s_getVIPLevel(userid)
	local robots = self.robotids[viplevel + 1]
	if robots ~= nil then
		s_copyClearNpcByBaseID(userid, 60, 1202, sceneid, robots)
	end
end

function CopySceneInfo_1202:OnCopySceneVarChange(sceneid, key1, key2, newvalue, oldvalue)
	if key1 == self.refreshMonsterVar[1] and key2 == self.refreshMonsterVar[2] then
		...
		for _,monster in ipairs(monsterdatas) do
			if type(monster) == "table" and monster.montype == 2 then
				s_sysSummon(monster.monid, sceneid, 11 + math.random(0,10) - 5, 9 + math.random(0,10) - 5, 0, 1)
			end
		end
		local monstercount = s_getCopyEntryCount(sceneid, SCENE_ENTRY_NPC)
		s_sendCopyZoneOne(sceneid,tostring(monstercount))
...
function CopySceneInfo_1202_MyNpcDeadNoraml(userid, npcid)
	local sceneid = s_getValue(SCENE_ENTRY_NPC, npcid, VALUE_TYPE_SCENE_ID)
	local monstercount = s_getCopyEntryCount(sceneid, SCENE_ENTRY_NPC)
	s_sendCopyZoneOne(sceneid,tostring(monstercount))
end

function CopySceneInfo_1202:RegisterNpcDead()
	local monsterdatas = CoinCopyNpcDataBase
	for _,monster in ipairs(monsterdatas) do
		if type(monster) == "table" and monster.montype == 2 then
			RegisterMyNpcDeadEvent(monster.monid,"CopySceneInfo_1202_MyNpcDeadNoraml($1,$2)")
		end
	end
end

--玩家离开副本
function CopySceneInfo_1202:OnPlayerLeaveCopyScene(sceneid, userid)
    self:ClearRobot(sceneid, userid)
end

CopySceneInfo_1202:RegisterNpcDead()
