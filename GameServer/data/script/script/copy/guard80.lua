...
--副本创建初始化
function CopySceneInfo_12:InitCopyScene(cid)
...
function CopySceneInfo_12:OnPlayerIntoCopyScene(cid,uid)
...
function CopySceneInfo_12:OnPlayerLeaveCopyScene(cid,uid)
...
CopySceneInfo_12:refreshScore(uid)
end
]]--

function OnLoopCopyScene_1Sec_12(ctype,cid)
	...
	if begin > 0 then
		...
		if begin == 1 then
			CopySceneInfo_12:createMonster(cid,0)
		end
		return
	end
	...
	if pass % 60 == 0 then
		...
		local turn = pass / 60
		if turn < 15 then
			CopySceneInfo_12:createMonster(cid,turn)
		end
		if turn <= 15 then
			local list = s_getCopyEntry(cid,SCENE_ENTRY_PLAYER)
			for i = 1,table.getn(list), 2 do
				...
				CopySceneInfo_12:refreshScore(list[i+1])
			end
			if turn == 15 then
				local list = s_getCopyEntry(cid,SCENE_ENTRY_PLAYER)
				for i = 1,table.getn(list), 2 do
	...
	if s_getSceneVar(cid,0,2) == 0 then
		CopySceneInfo_12:checkOver(cid)
	end
end

function CopySceneInfo_12:checkOver(cid)
	--检查是否通关
	local cur_time = s_getCurTime(TIME_ALLSECONDS)
	local starttime = s_getSceneVar(cid,0,200)
...
function CopySceneInfo_12:jumpTarget(baseid,uid)
	local cid = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_SCENE_ID)
	if baseid == 145 then
...
function CopySceneInfo_12:initMonster(cid,data)
...
function CopySceneInfo_12:createMonster(cid,turn)
	--turn = 4
...
--第三种
{
	...
	CopySceneInfo_12:initMonster(cid,monster[rand][turn+1])
		...
		if rand1 <= rate then
			...
			if turn+1 == 15 then
				for i=1,10-killernum do
					local npcid = s_sysSummon(24554,cid,posx,posy)
		...
		if rand1 <= rate then
			...
			if turn+1 == 15 then
				for i=1,10-superkillernum do
					local npcid = s_sysSummon(24555,cid,posx,posy)
...
function CopySceneInfo_12:dead145(thisid)
...
function CopySceneInfo_12:dead146(thisid)
	local ctype = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_TYPE)
	if ctype == 5 then
		local cid = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_ID)
...
function CopySceneInfo_12:dead147(thisid)
	local ctype = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_TYPE)
	if ctype == 5 then
		local cid = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_ID)
...
function CopySceneInfo_12:dead148(thisid)
	local ctype = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_TYPE)
	if ctype == 5 then
		local cid = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_ID)
...
function CopySceneInfo_12:dead149(thisid)
	local ctype = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_TYPE)
	if ctype == 5 then
		local cid = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_ID)
...
function CopySceneInfo_12:dead150(thisid)
	local ctype = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_TYPE)
	if ctype == 5 then
		local cid = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_ID)
		s_copyInfo(cid,"B路二号守卫被击败，赶快前去帮助。",ChatPos_Sys + ChatPos_Important)
...
function CopySceneInfo_12:dead347(thisid)
	local ctype = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_TYPE)
	if ctype == 5 then
		local cid = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_ID)
...
function CopySceneInfo_12:dead348(thisid)
	local ctype = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_TYPE)
	if ctype == 5 then
		local cid = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_ID)
		s_copyInfo(cid,"C路二号守卫被击败，赶快前去帮助。",ChatPos_Sys + ChatPos_Important)
...
function CopySceneInfo_12:dead349(thisid)
	local ctype = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_TYPE)
	if ctype == 5 then
		local cid = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_ID)
...
function CopySceneInfo_12:refreshScore(uid)
	local exp = s_getVar(uid,101,20)
	local score = s_getVar(uid,101,21)
...
--增加守卫要塞副本的排队
s_createQueueOneGroup(13,12,0,111,37,217,"守卫要塞81级",6,0,100)
