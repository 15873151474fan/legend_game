...
CopySceneInfo_36 = {}
--副本创建初始化
function CopySceneInfo_36:InitCopyScene(cid)
	s_setSceneVar(cid,0,0,5)
	s_setSceneVar(cid,0,1,s_getCurTime(TIME_ALLSECONDS))
end

function CopySceneInfo_36:OnPlayerIntoCopyScene(cid,uid)
...
function CopySceneInfo_36:OnPlayerRelive(uid, rtype)
	if rtype == 3 or rtype == 4 then
		s_goto(SCENE_ENTRY_PLAYER,uid,125,198)
	end
end

function OnLoopCopyScene_1Sec_36(ctype,cid)
	local last = s_getCurTime(TIME_ALLSECONDS) - s_getSceneVar(cid,0,1)
	...
	if begin > 0 then
		...
		if begin == 1 then
			...
			CopySceneInfo_36:createMonster(cid)
			local last = s_getCurTime(TIME_ALLSECONDS) - s_getSceneVar(cid,0,1)
			s_sendCopyInfo(cid,180,30*60 - last,"杀死电锯改进型")
			return
		end
...
function CopySceneInfo_36:createMonster(cid)
	local monster = {
		{22483,124,150,2,15,1},		{22480,148,96,10,10,1},
		{22481,148,96,10,10,5},		{22480,144,79,10,10,1},		{22481,144,79,10,10,5},
		{22480,103,79,10,10,1},		{22481,103,79,10,10,5},		{22480,97,97,10,10,1},
		{22481,97,97,10,10,5},		{22480,123,97,10,10,1},		{22481,123,97,10,10,5},
		{22480,122,55,5,10,2},		{22481,122,55,5,10,2}, }
...
function CopySceneInfo_36:dead22483(uid)--看门boss
	...
	if ctype >= 30 and ctype <= 39 then
		...
		if s_getSceneVar(cid,0,3) ==0 then
			local npcid = s_sysSummon(22482,cid,122,53)
...
function CopySceneInfo_36:dead22482(uid) --最终boss
	local cid = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_SCENE_ID)
...
-- RegisterMyNpcDeadEvent(22482,"CopySceneInfo_36:dead22482($1)")
-- RegisterMyNpcDeadEvent(22483,"CopySceneInfo_36:dead22483($1)")
