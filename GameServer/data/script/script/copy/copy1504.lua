...
function CopySceneInfo_1504:InitCopyScene(cid)
...
function CopySceneInfo_1504:ClearNpc(cid)
     --清空npc
...
--玩家复活
function CopySceneInfo_1504:OnPlayerRelive(uid, rtype)
...
--玩家离开副本
function CopySceneInfo_1504:OnPlayerLeaveCopyScene(sceneid, uid)
...
function OnLoopCopyScene_1Sec_1504(ctype,cid)
	...
	if s_getVar(uid,140,93) == 2 then 
		if curtime >= s_getSceneVar(cid,1,success_over_time) then	
			...
			CopySceneInfo_1504:ClearNpc(cid)
			s_clearCopy(cid)	
		end
	end
...
function CopySceneInfo_1504_MyNpcDeadNoraml(uid, npcid)
...
function CopySceneInfo_1504:RegisterNpcDead()
	local monsterdatas = SceneMonsterDataBase
	for _,monster in ipairs(monsterdatas) do
		if type(monster) == "table" and monster.dwID == 4 then
			RegisterMyNpcDeadEvent(monster.monsterid,"CopySceneInfo_1504_MyNpcDeadNoraml($1,$2)")
		end
	end
end

CopySceneInfo_1504:RegisterNpcDead()
