...
--副本创建初始化
function CopySceneInfo_11:InitCopyScene(cid)
...
function CopySceneInfo_11:OnPlayerIntoCopyScene(cid,uid)
...
function CopySceneInfo_11:OnPlayerLeaveCopyScene(cid,uid)
...
CopySceneInfo_11:refreshScore(uid)
end
]]--

function OnLoopCopyScene_1Sec_11(ctype,cid)
	...
	if begin > 0 then
		...
		if begin == 1 then
			CopySceneInfo_11:createMonster(cid,0)
		end
		return
	end
	...
	if pass % 60 == 0 then
		...
		local turn = pass / 60
		if turn < 15 then
			CopySceneInfo_11:createMonster(cid,turn)
		end
		if turn <= 15 then
			local list = s_getCopyEntry(cid,SCENE_ENTRY_PLAYER)
			for i = 1,table.getn(list), 2 do
				...
				CopySceneInfo_11:refreshScore(list[i+1])
			end
			if turn == 15 then
				local list = s_getCopyEntry(cid,SCENE_ENTRY_PLAYER)
				for i = 1,table.getn(list), 2 do
	...
	if s_getSceneVar(cid,0,2) == 0 then
		CopySceneInfo_11:checkOver(cid)
	end
end

function CopySceneInfo_11:checkOver(cid)
	--检查是否通关
	local cur_time = s_getCurTime(TIME_ALLSECONDS)
	local starttime = s_getSceneVar(cid,0,200)
	...
	if s_getValue(SCENE_ENTRY_NPC,guardid,VALUE_TYPE_HP) ~= 0 then
		if cur_time > starttime + 15 * 60 then
			...
			if s_getSceneVar(cid,0,109) >= 5 and s_getSceneVar(cid,0,110) >= 5 and s_getSceneVar(cid,0,114) == 0 then
				local npcid = s_sysSummon(24518,cid,57,219)
...
function CopySceneInfo_11:jumpTarget(baseid,uid)
	local cid = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_SCENE_ID)
	if baseid == 145 then
...
function CopySceneInfo_11:initMonster(cid,data)
...
function CopySceneInfo_11:createMonster(cid,turn)
	--turn = 4
...
--第三种
{
	...
	CopySceneInfo_11:initMonster(cid,monster[rand][turn+1])
		...
		if rand1 <= rate then
			...
			if turn+1 == 15 then
				for i=1,10-killernum do
					local npcid = s_sysSummon(24516,cid,posx,posy)
		...
		if rand1 <= rate then
			...
			if turn+1 == 15 then
				for i=1,10-superkillernum do
					local npcid = s_sysSummon(24517,cid,posx,posy)
...
function CopySceneInfo_11:dead145(thisid)
...
function CopySceneInfo_11:dead146(thisid)
	local ctype = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_TYPE)
	if ctype == 5 then
		local cid = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_ID)
...
function CopySceneInfo_11:dead147(thisid)
	local ctype = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_TYPE)
	if ctype == 5 then
		local cid = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_ID)
...
function CopySceneInfo_11:dead148(thisid)
	local ctype = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_TYPE)
	if ctype == 5 then
		local cid = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_ID)
...
function CopySceneInfo_11:dead149(thisid)
	local ctype = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_TYPE)
	if ctype == 5 then
		local cid = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_ID)
...
function CopySceneInfo_11:dead150(thisid)
	local ctype = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_TYPE)
	if ctype == 5 then
		local cid = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_ID)
		s_copyInfo(cid,"B路二号守卫被击败，赶快前去帮助。",ChatPos_Sys + ChatPos_Important)
...
function CopySceneInfo_11:dead347(thisid)
	local ctype = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_TYPE)
	if ctype == 5 then
		local cid = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_ID)
...
function CopySceneInfo_11:dead348(thisid)
	local ctype = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_TYPE)
	if ctype == 5 then
		local cid = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_ID)
		s_copyInfo(cid,"C路二号守卫被击败，赶快前去帮助。",ChatPos_Sys + ChatPos_Important)
...
function CopySceneInfo_11:dead349(thisid)
	local ctype = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_TYPE)
	if ctype == 5 then
		local cid = s_getValue(SCENE_ENTRY_NPC,thisid,VALUE_TYPE_SCENE_ID)
...
function CopySceneInfo_11:refreshScore(uid)
	local exp = s_getVar(uid,101,20)
	local score = s_getVar(uid,101,21)
...
--增加守卫要塞副本的排队
s_createQueueOneGroup(13,11,0,111,37,217,"守卫要塞61级",6,0,100)
