--翅膀礼盒
function OnUseItemEvent_4010(uid,thisid)
	if s_deleteItemByThisID(uid,thisid,1,"翅膀礼盒10") ~= VALUE_OK then
			return VALUE_FAIL
	end

	local wing_id=291001		--翅膀baseid
	local yulingdan=10095--羽灵丹 baseid
	local yulingdan_num=10		--羽灵丹数量
	...
	if ret == 1 then
		...
		if ret2 == 1 then
			s_sysInfo("激活了十阶翅膀",ChatPos_Tips)
		end
	end
	return VALUE_OK
end
