...
CopySceneInfo_33 = {}
--副本创建初始化
function CopySceneInfo_33:InitCopyScene(cid)
	s_setSceneVar(cid,0,0,5)
	s_setSceneVar(cid,0,1,s_getCurTime(TIME_ALLSECONDS))
end

function CopySceneInfo_33:OnPlayerIntoCopyScene(cid,uid)
...
function CopySceneInfo_33:OnPlayerRelive(uid, rtype)
	if rtype == 3 or rtype == 4 then
		s_goto(SCENE_ENTRY_PLAYER,uid,125,198)
	end
end

function OnLoopCopyScene_1Sec_33(ctype,cid)
	local last = s_getCurTime(TIME_ALLSECONDS) - s_getSceneVar(cid,0,1)
	...
	if begin > 0 then
		...
		if begin == 1 then
			...
			CopySceneInfo_33:createMonster(cid)
			local last = s_getCurTime(TIME_ALLSECONDS) - s_getSceneVar(cid,0,1)
			s_sendCopyInfo(cid,180,30*60 - last,"杀死电锯改进型")
			return
		end
...
function CopySceneInfo_33:createMonster(cid)
	local monster = {
		{22471,124,150,2,15,1},		{22468,148,96,10,10,1},
		{22469,148,96,10,10,5},		{22468,144,79,10,10,1},		{22469,144,79,10,10,5},
		{22468,103,79,10,10,1},		{22469,103,79,10,10,5},		{22468,97,97,10,10,1},
		{22469,97,97,10,10,5},		{22468,123,97,10,10,1},		{22469,123,97,10,10,5},
		{22468,122,55,5,10,2},		{22469,122,55,5,10,2}, }
...
function CopySceneInfo_33:dead22471(uid)--看门boss
	...
	if ctype >= 30 and ctype <= 39 then
		...
		if s_getSceneVar(cid,0,3) ==0 then
			local npcid = s_sysSummon(22470,cid,122,53)
...
function CopySceneInfo_33:dead22470(uid) --最终boss
	local cid = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_SCENE_ID)
...
-- RegisterMyNpcDeadEvent(22470,"CopySceneInfo_33:dead22470($1)")
-- RegisterMyNpcDeadEvent(22471,"CopySceneInfo_33:dead22471($1)")
