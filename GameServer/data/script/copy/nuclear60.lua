...
function CopySceneInfo_31:OnPlayerRelive(uid, rtype)
	if rtype == 3 or rtype == 4 then
		s_goto(SCENE_ENTRY_PLAYER,uid,125,198)
	end
end

function OnLoopCopyScene_1Sec_31(ctype,cid)
	local last = s_getCurTime(TIME_ALLSECONDS) - s_getSceneVar(cid,0,1)
	...
	if begin > 0 then
		...
		if begin == 1 then
			...
			CopySceneInfo_31:createMonster(cid)
			local last = s_getCurTime(TIME_ALLSECONDS) - s_getSceneVar(cid,0,1)
			s_sendCopyInfo(cid,180,30*60 - last,"杀死电锯改进型")
			return
		end
...
function CopySceneInfo_31:createMonster(cid)
	local monster = {
		{22461,124,150,2,15,1},		{22458,148,96,10,10,1},
		{22459,148,96,10,10,5},		{22458,144,79,10,10,1},		{22459,144,79,10,10,5},
		{22458,103,79,10,10,1},		{22459,103,79,10,10,5},		{22458,97,97,10,10,1},
		{22459,97,97,10,10,5},		{22458,123,97,10,10,1},		{22459,123,97,10,10,5},
		{22458,122,55,5,10,2},		{22459,122,55,5,10,2}, }
...
function CopySceneInfo_31:dead22461(uid)--看门boss
	...
	if ctype >= 30 and ctype <= 39 then
		...
		if s_getSceneVar(cid,0,3) ==0 then
			local npcid = s_sysSummon(22460,cid,122,53)
...
function CopySceneInfo_31:dead22460(uid) --最终boss
	local cid = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_SCENE_ID)
...
-- RegisterMyNpcDeadEvent(22460,"CopySceneInfo_31:dead22460($1)")
-- RegisterMyNpcDeadEvent(22461,"CopySceneInfo_31:dead22461($1)")
