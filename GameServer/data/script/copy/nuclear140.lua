...
CopySceneInfo_39 = {}
--副本创建初始化
function CopySceneInfo_39:InitCopyScene(cid)
	s_setSceneVar(cid,0,0,5)
	s_setSceneVar(cid,0,1,s_getCurTime(TIME_ALLSECONDS))
end

function CopySceneInfo_39:OnPlayerIntoCopyScene(cid,uid)
...
function CopySceneInfo_39:OnPlayerRelive(uid, rtype)
	if rtype == 3 or rtype == 4 then
		s_goto(SCENE_ENTRY_PLAYER,uid,125,198)
	end
end

function OnLoopCopyScene_1Sec_39(ctype,cid)
	local last = s_getCurTime(TIME_ALLSECONDS) - s_getSceneVar(cid,0,1)
	...
	if begin > 0 then
		...
		if begin == 1 then
			...
			CopySceneInfo_39:createMonster(cid)
			local last = s_getCurTime(TIME_ALLSECONDS) - s_getSceneVar(cid,0,1)
			s_sendCopyInfo(cid,180,30*60 - last,"杀死电锯改进型")
			return
		end
...
function CopySceneInfo_39:createMonster(cid)
	local monster = {
		{22495,124,150,2,15,1},		{22492,148,96,10,10,1},
		{22493,148,96,10,10,5},		{22492,144,79,10,10,1},		{22493,144,79,10,10,5},
		{22492,103,79,10,10,1},		{22493,103,79,10,10,5},		{22492,97,97,10,10,1},
		{22493,97,97,10,10,5},		{22492,123,97,10,10,1},		{22493,123,97,10,10,5},
		{22492,122,55,5,10,2},		{22493,122,55,5,10,2}, }
...
function CopySceneInfo_39:dead22495(uid)--看门boss
	...
	if ctype >= 30 and ctype <= 39 then
		...
		if s_getSceneVar(cid,0,3) ==0 then
			local npcid = s_sysSummon(22494,cid,122,53)
...
function CopySceneInfo_39:dead22494(uid) --最终boss
	local cid = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_SCENE_ID)
...
-- RegisterMyNpcDeadEvent(22494,"CopySceneInfo_39:dead22494($1)")
-- RegisterMyNpcDeadEvent(22495,"CopySceneInfo_39:dead22495($1)")
