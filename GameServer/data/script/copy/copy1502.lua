...
function CopySceneInfo_1502:InitCopyScene(cid)
...
function CopySceneInfo_1502:ClearNpc(cid)
     --清空npc
...
--玩家复活
function CopySceneInfo_1502:OnPlayerRelive(uid, rtype)
...
--玩家离开副本
function CopySceneInfo_1502:OnPlayerLeaveCopyScene(sceneid, uid)
...
function OnLoopCopyScene_1Sec_1502(ctype,cid)
	...
	if s_getVar(uid,140,93) == 2 then 
		if curtime >= s_getSceneVar(cid,1,success_over_time) then	
			...
			CopySceneInfo_1502:ClearNpc(cid)
			s_clearCopy(cid)	
		end
	end
...
function CopySceneInfo_1502_MyNpcDeadNoraml(uid, npcid)
...
function CopySceneInfo_1502:RegisterNpcDead()
	local monsterdatas = SceneMonsterDataBase
	for _,monster in ipairs(monsterdatas) do
		if type(monster) == "table" and monster.dwID == 2 then
			RegisterMyNpcDeadEvent(monster.monsterid,"CopySceneInfo_1502_MyNpcDeadNoraml($1,$2)")
		end
	end
end

CopySceneInfo_1502:RegisterNpcDead()
