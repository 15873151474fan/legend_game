...
function CopySceneInfo_1503:InitCopyScene(cid)
	s_debug(">>>>>>>>>获得加速buff加速,速度npcid:"..cid)
	--s_setSceneVar(cid,1,defencetower_over_time,s_getCurTime(TIME_ALLSECONDS) + 30)
	--[[local chapterid = 0
...
function CopySceneInfo_1503:ClearNpc(cid)
     --清空npc
...
--玩家复活
function CopySceneInfo_1503:OnPlayerRelive(uid, rtype)
...
--玩家离开副本
function CopySceneInfo_1503:OnPlayerLeaveCopyScene(sceneid, uid)
...
function OnLoopCopyScene_1Sec_1503(ctype,cid)
	...
	if s_getVar(uid,140,93) == 2 then 
		if curtime >= s_getSceneVar(cid,1,success_over_time) then	
			...
			CopySceneInfo_1503:ClearNpc(cid)
			s_clearCopy(cid)	
		end
	end
...
function CopySceneInfo_1503_MyNpcDeadNoraml(uid, npcid)
	...
	if monstercount == 0 then
		MaterialSuccess(uid)
		s_setSceneVar(sceneid,1,success_over_time,s_getCurTime(TIME_ALLSECONDS) + 60)
...
function CopySceneInfo_1503:RegisterNpcDead()
	local monsterdatas = SceneMonsterDataBase
	for _,monster in ipairs(monsterdatas) do
		if type(monster) == "table" and monster.dwID == 3 then
			RegisterMyNpcDeadEvent(monster.monsterid,"CopySceneInfo_1503_MyNpcDeadNoraml($1,$2)")
		end
	end
end

CopySceneInfo_1503:RegisterNpcDead()
