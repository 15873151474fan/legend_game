...
CopySceneInfo_35 = {}
--副本创建初始化
function CopySceneInfo_35:InitCopyScene(cid)
	s_setSceneVar(cid,0,0,5)
	s_setSceneVar(cid,0,1,s_getCurTime(TIME_ALLSECONDS))
end

function CopySceneInfo_35:OnPlayerIntoCopyScene(cid,uid)
...
function CopySceneInfo_35:OnPlayerRelive(uid, rtype)
	if rtype == 3 or rtype == 4 then
		s_goto(SCENE_ENTRY_PLAYER,uid,125,198)
	end
end

function OnLoopCopyScene_1Sec_35(ctype,cid)
	local last = s_getCurTime(TIME_ALLSECONDS) - s_getSceneVar(cid,0,1)
	...
	if begin > 0 then
		...
		if begin == 1 then
			...
			CopySceneInfo_35:createMonster(cid)
			local last = s_getCurTime(TIME_ALLSECONDS) - s_getSceneVar(cid,0,1)
			s_sendCopyInfo(cid,180,30*60 - last,"杀死电锯改进型")
			return
		end
...
function CopySceneInfo_35:createMonster(cid)
	local monster = {
		{22479,124,150,2,15,1},		{22476,148,96,10,10,1},
		{22477,148,96,10,10,5},		{22476,144,79,10,10,1},		{22477,144,79,10,10,5},
		{22476,103,79,10,10,1},		{22477,103,79,10,10,5},		{22476,97,97,10,10,1},
		{22477,97,97,10,10,5},		{22476,123,97,10,10,1},		{22477,123,97,10,10,5},
		{22476,122,55,5,10,2},		{22477,122,55,5,10,2}, }
...
function CopySceneInfo_35:dead22479(uid)--看门boss
	...
	if ctype >= 30 and ctype <= 39 then
		...
		if s_getSceneVar(cid,0,3) ==0 then
			local npcid = s_sysSummon(22478,cid,122,53)
...
function CopySceneInfo_35:dead22478(uid) --最终boss
	local cid = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_SCENE_ID)
...
-- RegisterMyNpcDeadEvent(22478,"CopySceneInfo_35:dead22478($1)")
-- RegisterMyNpcDeadEvent(22479,"CopySceneInfo_35:dead22479($1)")
