...
function CopySceneInfo_233:InitCopyScene(cid)
...
function CopySceneInfo_233:ClearNpc(cid)
    --清空npc
    local list = s_getCopyEntry(cid,SCENE_ENTRY_NPC)
    for i = 1,table.getn(list), 2 do --清理NPC
        s_clearNpc(list[i+1])
...
--		s_debug("奖励波数--------"..s_getVar(uid,470,1))
    ...
    --开始刷怪
    if s_getSceneVar(cid,1,defencetower_isbegin) == 1 and s_getSceneVar(cid,1,defencetower_is_over) == 0 and s_getSceneVar(cid,1,defencetower_monster_current) <= 6 and curtime < s_getSceneVar(cid,1,defencetower_over_time) and s_getVar(uid,476,1) == 0 then
        if s_getSceneVar(cid,1,defencetower_monster_current) == 0 then
            s_setSceneVar(cid,1,defencetower_monster_current,1)
        end
        CopySceneInfo_233:beginRefreshMonster(s_getSceneVar(cid,1,defencetower_monster_current),cid)
    ...
    if monstercount == 0 and s_getSceneVar(cid,1,defencetower_monster_current) ~= 0 and s_getSceneVar(cid,1,defencetower_monster_current) <= 6 and curtime < s_getSceneVar(cid,1,defencetower_over_time) then
            ...
            if current > 6 then
		s_debug("停止刷新")
		if(s_getVar(uid,470,1) == 5) then
			s_setVar(uid,470,1,6)
		end
		s_setSceneVar(cid,1,defencetower_is_over,1)
...
--玩家复活
function CopySceneInfo_233:OnPlayerRelive(uid, rtype)
...
--]]
--召唤守卫
function CopySceneInfo_233:RefreshNpc(typeid,cid,uid)
	s_debug("召唤兽为")
	if typeid == 4 then
		s_getVar(uid,474,1)
...
--玩家进入副本提示
function CopySceneInfo_233:OnPlayerIntoCopyScene(cid,uid)
	s_addDailyTargetTime(uid, enumDailySubTargetType_CopySceneExp, 1)
        local list = s_getCopyEntry(cid,SCENE_ENTRY_PLAYER)
        local uid = 0
        for i = 1,table.getn(list), 2 do
...
function CopySceneInfo_233:beginRefreshMonster(curnumber,cid)
...
function CopySceneInfo_233:dead(cid,npcid)
        local list = s_getCopyEntry(cid,SCENE_ENTRY_NPC)
        local num = 0
...
for i=0,24 do
   local id = 22100 + i
   RegisterCopyNpcDeadEvent(22100+i,233,0,"CopySceneInfo_233:dead($1,$2)")
end
