...
function getequipwashrandomrate(ratenum)
	local sratetinfo = EquipWashRandomDataBase
...
function RessetEauipAllAttrZero(uid,equipid)
...
function ResetbindTypeAndData(uid,equipid)
...
function RessetEauipBindAttrZero(uid,equipid)
...
--生成属性值
function editwashtabledAddAttr(attr,minvalue,maxvalue,final,key)
	math.randomseed(tostring(os.time()):reverse():sub(1, 7))
	local ratenum = math.random(1,1000000) --配置表中最大值
	local rateinfo = getequipwashrandomrate(ratenum)
	local finalnum = 0
...
--获得属性数量 成对代表一条存在
function GetPolisthedAttrNum( uid,sstable )
	local typettb = Polished_Type
...
--生成随机的概率
function getratenumfromrand( washtable, ratenum, maxgetnum )
...
--编辑最终属性table
function EditfinalAttrAddInfo(uid,equipid,bindtype,totalinfo)
		...
		s_debugUser(uid,"EditHorsefinalAttrAddInfo equipid="..equipid.." tmpattr="..tmpattr.." tmpminvalue="..tmpminvalue.." tmpmaxvalue="..tmpmaxvalue.." tmpfinal="..tmpfinal.." tmpkey="..tmpkey)
		return tmpattr,tmpminvalue,tmpmaxvalue,tmpfinal,tmpkey
end


--洗练装备
function UserPolishSpecificEquipAttr(uid,equipid)
	 ...
	 if 0 == s_checkItem(uid,10095,washcost,0) then
	 ...
	 ret = s_deleteItemByBaseID(uid,10095,washcost,"洗练装备扣除材料",0)
	 if ret == nil or ret == 0 then
	 ...
	 --重置多有绑定属性
	 RessetEauipBindAttrZero(uid,equipid)
	...
	for k,v in pairs(newwashinfo)do
	    if v > 0 and k ~= "id" and k~="washNum" and k~="cost" and k~="lockCost" and k~="transformType" and k~="transformNum" and k~="maxGrade" and k~="maxPower" then
				...
				if tmp ~= nil then
				 --s_debugUser(uid,"洗练::tmp.attr=%s,minvalue=%u,maxvalue=%u,final=%u,index=%u",k,tmp.minvalue,tmp.maxvalue,tmp.final,index)
			        end
	...
	for k,v in pairs(getwashinfo) do 
		for m,n in pairs(Equipwashtype) do
			if v.attr == m then
				v.key = n
			end
		end
	...
	if bindtype1 > 0 then
		local tmp = {}
		local tattr,tmpminvalue,tmaxvalue,tfinal,tkey = EditfinalAttrAddInfo(uid,equipid,bindtype1,getwashinfo)
		tmp.attr = tattr
	...
	if bindtype2 > 0 then
		local tmp = {}
		local tattr,tmpminvalue,tmaxvalue,tfinal,tkey = EditfinalAttrAddInfo(uid,equipid,bindtype2,getwashinfo)
		tmp.attr = tattr
		tmp.minvalue = tmpminvalue
		tmp.maxvalue = tmaxvalue
		tmp.final = tfinal
	...
	if bindtype3 > 0 then
		local tmp = {}
		local tattr,tmpminvalue,tmaxvalue,tfinal,tkey = EditfinalAttrAddInfo(uid,equipid,bindtype3,getwashinfo)
		tmp.attr = tattr
		tmp.minvalue = tmpminvalue
		tmp.maxvalue = tmaxvalue
		tmp.final = tfinal
	...
	if bindtype4 > 0 then
		local tmp = {}
		local tattr,tmpminvalue,tmaxvalue,tfinal,tkey = EditfinalAttrAddInfo(uid,equipid,bindtype4,getwashinfo)
		tmp.attr = tattr
		tmp.minvalue = tmpminvalue
		tmp.maxvalue = tmaxvalue
		tmp.final = tfinal
	...
	if bindtype5 > 0 then
		local tmp = {}
		local tattr,tmpminvalue,tmaxvalue,tfinal,tkey = EditfinalAttrAddInfo(uid,equipid,bindtype5,getwashinfo)
		tmp.attr = tattr
		tmp.minvalue = tmpminvalue
		tmp.maxvalue = tmaxvalue
		tmp.final = tfinal
	...
	if bindtype6 > 0 then
		local tmp = {}
		local tattr,tmpminvalue,tmaxvalue,tfinal,tkey = EditfinalAttrAddInfo(uid,equipid,bindtype6,getwashinfo)
		tmp.attr = tattr
		tmp.minvalue = tmpminvalue
		tmp.maxvalue = tmaxvalue
		tmp.final = tfinal
	...
	local polishtype = GetPolisthedAttrNum(uid,getwashinfo)
    ...
    for k,v in pairs(willwash) do
    	for i,j in pairs(getwashinfo) do
    		if v == j.attr then
    			...
    			tmp.final = j.final
    			tmp.key = j.key
    			--s_debugUser(uid,"tmpppppp...,attr : %s , maxvalue : %u , final : %u , key : %u",tmp.attr,tmp.maxvalue,tmp.final,tmp.key)
	...
	local ratewashtable = Equipwashattr
	for k,v in pairs(ratewashtable) do
	    ratewashtable[""..k] = 0            
	end 


	local finalratebatle = getratenumfromrand(ratewashtable,ratemaxnum,maxgetnum)

		--s_debugUser(uid," RateNUM  ... RRRRRRRR : %u , %u",ratemaxnum,maxgetnum)
	for k,v in pairs(willwashdetail) do
		local rfinal,rret =math.modf((v.maxvalue * finalratebatle[""..v.attr])/ratemaxnum)
	    		...
	    		--s_debugUser(uid," 结果 ... Attr : %s , key : %u , final  %u ,maxvalue : %u, ratenum : %u",tmp.attr,tmp.key,tmp.final,tmp.maxvalue,finalratebatle[""..v.attr])
	end

	local washtypes = Equipwashtype
	local configgrade = WashGradeDataBase
	...
	for k,v in ipairs(finalinfo) do    
		if v.maxvalue > 0 then 		
			...
			--s_debugUser(uid,"洗练:::v.maxvalue=%u,tmprate=%u,final=%u,key=%u,ratio=%u,totalrate=%u",v.maxvalue,tmprate,v.final,v.key,powerinfo[v.key].ratio,totalrate);
		end
	end
	...
	--	local spreadstr = "天降祥瑞，震动寰宇。恭喜[#8]"..username.."[-]，功夫不负有心人，将[#14]"..armname.."[-]成功洗炼为[#14]"..ntype.."[-]"	
	--	s_worldInfo(spreadstr,ChatPos_Sys + ChatPos_Important)
	--end
...
--[[
    ...
    for k,v in pairs(willwashdetail) do
    	local rret,rfinal = editwashtabledAddAttr(v.attr,v.minvalue,v.maxvalue,v.final,v.key)
	...
	if finaldata ~= nil then
		...
		s_sendEquipWashResult(uid,equipid,1,1)
	end
end
--替换洗练属性
function UserSaveSpecificEquipAttr(uid,equipid)
	...
	local Tempdatascore = s_getItemValue(uid,equipid,1063)	    
	...
	RessetEauipAllAttrZero(uid,equipid)
	RessetEauipBindAttrZero(uid,equipid);
	...
	local finalinfo = {}
	local washgrade = {}
	local washdata = EquipWashDataBase.query(equbaseid)
	...
	s_debugUser(uid," 洗练结果...,washlevel : %u, Finalscore : %u",washlevel,finalscore)
	if washlevel >= 8 then
			local ntype = washlevelname
			local username = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_NAME)
			local armname = s_getItemValue(uid,equipid,-6)
			...
			if washlevel <=10 then
				spreadstr = "天降祥瑞，震动寰宇。恭喜[#3]"..username.."[-]，功夫不负有心人，将[#14]"..armname.."[-]成功洗炼为[#4]"..ntype.."[-]"	
			end
			if washlevel >10 and washlevel <=13 then
				spreadstr = "天降祥瑞，震动寰宇。恭喜[#3]"..username.."[-]，功夫不负有心人，将[#14]"..armname.."[-]成功洗炼为[#10]"..ntype.."[-]"	
			end
			if washlevel >13 and washlevel <=16 then
				spreadstr = "天降祥瑞，震动寰宇。恭喜[#3]"..username.."[-]，功夫不负有心人，将[#14]"..armname.."[-]成功洗炼为[#5]"..ntype.."[-]"
			end
		s_worldInfo(spreadstr,ChatPos_Sys + ChatPos_ImportantDown,1)
	end
   	s_setItemValue(uid,equipid,1062,Tempdatascore)

		--s_debugUser(uid," 洗练保存结果...,%u , %u , %u %u",uid,equipid,1062,Tempdatascore)
	if TempType1 > 0 then
	...
	ResetbindTypeAndData(uid,equipid)
	s_sendEquipWashResult(uid,equipid,2,1)
end
--绑定洗练属性
function UserBindSpecificEquipAttr(uid,equipid,bindtype)
	...
	s_sendEquipWashResult(uid,equipid,3,1)
end
--解绑指定属性
function UserDeBindSpecificEquipAttr(uid,equipid,bindtype)
	...
	s_sendEquipWashResult(uid,equipid,4,1)

end
--洗练属性继承
function UserReplaceSpecificEquipAttr(uid,equipsrcid,equidecid)
	...
	local speed = s_getItemValue(uid,equidecid,Item_Attribute_Polished_hoursespeed)	
	local totalcore = s_getItemValue(uid,equidecid,Item_Attribute_Polished_TotalScore)	
	local tempscore = s_getItemValue(uid,equidecid,Item_Attribute_Polished_TempScore)	
	...
	local Bind_Type6 = s_getItemValue(uid,equidecid,Item_Attribute_Polished_Bind_Type6)	 
   	...
   	if addhp > 0 or addsp > 0 or minpdam > 0 or maxpdam > 0 or minmdam > 0 or maxmdam > 0 or mintdam > 0 or maxtdam > 0 or minpdef > 0 or maxpdef > 0 or minmdef > 0 or maxmdef > 0 or luckyvalue > 0 or luckyper > 0 or luckydefvalue > 0 or luckydef > 0 or pdefper > 0 or mdefper > 0 or intervaldef > 0 or intervalvalue > 0 or intervalrecover > 0 or intervalvalueaddper > 0 or hpaddper > 0 or spaddper > 0 or minmonsteratt > 0 or maxmonsteratt > 0 or minmonsterdef > 0 or hurtper > 0 or speed > 0 then
	
		s_sysInfo(uid,"目标装备需要未进行洗炼!",ChatPos_Tips)
		return VALUE_FAIL
	end 

	 if s_getItemValue(uid,equidecid,-1) ~= s_getItemValue(uid,equipsrcid,-1) or
		s_getItemValue(uid,equidecid,-9) ~= s_getItemValue(uid,equipsrcid,-9) then
		s_sysInfo(uid,"装备的洗炼属性继承需要同职业同部位",ChatPos_Tips)
		return VALUE_FAIL
	end
	
	if decnewwashinfo.maxGrade < srcnewwashinfo.maxGrade  then
		s_sysInfo(uid,"无法将高品质装备的洗炼属性转移到低品质的装备上",ChatPos_Tips)
		return VALUE_FAIL
	end
	...
	local newwashinfo = EquipWashDataBase.query(decequbaseid)
	local powerinfo = EquipWashScoreDataBase
	...
	local srspeed = s_getItemValue(uid,equipsrcid,Item_Attribute_Polished_hoursespeed)	
	local srtotalscore = s_getItemValue(uid,equipsrcid,Item_Attribute_Polished_TotalScore)	
	local srtempscore = s_getItemValue(uid,equipsrcid,Item_Attribute_Polished_TempScore)	
	...
	s_setItemValue(uid,equidecid,Item_Attribute_Polished_hoursespeed,srspeed)	
	s_setItemValue(uid,equidecid,Item_Attribute_Polished_TotalScore,ftotalscore)	
	s_setItemValue(uid,equidecid,Item_Attribute_Polished_TempScore,srtempscore)	

	--重置旧装备属性为0
	--RessetEauipAllAttrZero(uid,equipsrcid)
	--ResetbindTypeAndData(uid,equipsrcid)
	--RessetEauipBindAttrZero(uid,equipsrcid)
	for i=1,63,1 do 
		if i < 39 or i > 40 then
			s_setItemValue(uid,equipsrcid,1000+i,0)
		end
	end
	s_sendEquipWashResult(uid,equidecid,5,1)
	s_sysInfo(uid,"洗炼继承完成",ChatPos_Tips)

end
--清除绑定属性1
function UserClearBindPolishSpecificEquipAttr(uid,equipid)

  	s_debugUser(uid,"UserClearBindPolishSpecificEquipAttr...equipid : "..equipid)
	 local equbaseid = s_getitembaseid(uid,equipid)	
	 if equbaseid == nil then
		 s_debugUser(uid," Baseid 未找到信息 ...")
	...
	s_sendEquipWashResult(uid,equipsrcid,6,1)
